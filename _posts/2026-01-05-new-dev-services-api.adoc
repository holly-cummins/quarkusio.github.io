...
If you do decide to build against 3.28, add the following to the `pom.xml` of your runtime module:
...
With the old Dev Services model, you might customise your extensions's card with code something like this:
...
Instead, replace the `url` method with `dynamicUrlJsonRPCMethodName`, link:/guides/dev-ui#runtime-external-links[passing in an RPC method name] or a config property key.
...
----
    .dynamicUrlJsonRPCMethodName("devui-dev-services:devServicesConfig",
                        Map.of("name", FEATURE, "configKey", someProp))
----

From 3.32, you can also pass through parameters on the custom method call. For example,
...
This offers maximum flexibility, but does involve extra code. For the custom method case, add a build step to register the providing class:
