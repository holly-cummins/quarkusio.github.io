...
public class ConfigTablePostprocessor extends Postprocessor {
    ...
    public String process(Document document, String output) {
        ...
        if (docId == null || !docId.startsWith("page-with-config-")) {
            return output;
        }
        ...
        for (int id = 1; id <= maxId; id++) {
            ...
            int descIdx = indexOf(sb, collapsibleMarker);
            if (descIdx >= 0) {
                int rowIdx = lastIndexOf(sb, "<tr>", descIdx);
                if (rowIdx >= 0) {
                ...
                descIdx = indexOf(sb, nonCollapsibleMarker);
    ...
    private static int lastIndexOf(StringBuilder sb, String str, int fromIndex) {
