...
public class ConfigTableTreeprocessor extends Treeprocessor {
    ...
    public Document process(Document document) {
        ...
        for (StructuralNode node : tables) {
            ...
            if (table.hasRole("searchable") && table.getParent() instanceof StructuralNode) {
                StructuralNode parent = (StructuralNode) table.getParent();
                {
                    ...
                    if (tableIndex > 0) {
                        ...
                        Block newCaption = createBlock(parent, "paragraph",
                                captionContent
                                        + " <input type=\"search\" id=\"config-search-" + searchFieldId
                                        + "\" placeholder=\"FILTER CONFIGURATION\" disabled>",
                                new HashMap<>());
        ...
        if (rowId > 0) {
            document.setId("page-with-config-0");
